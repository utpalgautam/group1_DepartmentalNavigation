...
import { buildingFromFirestore, buildingToFirestore, BuildingModel, EntryPoint } from './models';
...
export const addBuilding = functions.https.onRequest((req, res) => {
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      const validatedEntryPoints: EntryPoint[] = entryPoints.map((ep: any, idx: number) => {
      ...
      const buildingData: BuildingModel = {
        name: name.trim(),
